...
static int rb_open(struct block_device *bdev, fmode_t mode)
{
    ...
    printk(KERN_INFO "Block driver: Inode number is %d\n", unit);
...
static int rb_transfer(struct request *req)
{
    ...
    rq_for_each_segment(bv, req, iter)
    {
        ...
        {
            printk(KERN_ERR "Block driver: Should never happen: "
        ...
        printk(KERN_DEBUG "Block driver: Sector Offset: %lld; Buffer: %p; Length: %d sectors\n",
    ...
    {
        printk(KERN_ERR "Block driver: bio info doesn't match with the request info");
        ret = -EIO;
    }
 ...
 *	в структуру block_device_operations заносятся операции для работы с файлами устройств с зарегистрированными старшими номерами
 * (очереди запросов)
 */
...
static int __init rb_init(void)
{
    ...
    /* Get Registered */
    rb_major = register_blkdev(rb_major, "blkDrv");
    if (rb_major <= 0)
    {
        printk(KERN_ERR "Block driver: Unable to get Major Number\n");
        ramdevice_cleanup();
        return -EBUSY;
    }
    ...
    {
        printk(KERN_ERR "Block driver: blk_init_queue failure\n");
        unregister_blkdev(rb_major, "blkDrv");
        ramdevice_cleanup();
        return -ENOMEM;
    }
    ...
    if (!rb_dev.rb_disk)
    {
        printk(KERN_ERR "Block driver: alloc_disk failure\n");
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Bakshenov.V, Ghazi.D, Klyuchnikov.V");
MODULE_DESCRIPTION("Second lab block driver.");
